...
products2 = [
    {
        ...
        'name': 'Raymond - Kurta',
        'price': 2000,
        'description': 'Classy and Traditional Look',
        'image': 'kurta.jpeg',
        'specifications': {
            'cloth' : 'Cotton blend'
...
products3=[
    {
        'id': 7,
        'name': 'Power of your subconsious mind',
        'price': 700,
        'description': 'By Joseph Murphy',
        'image': 'Book1.jpeg',
        'specifications': {
            'Purpose' : 'Spiritual Wisdom with scientific resarch'
    ...
    },
    {
        ...
        'price': 650,
        'description': 'By Warren Buffet',
        'image': 'Book2.jpeg',
        'specifications': {
            'Purpose' : "Importance of financial literacy"
        }
    }
]
...
@app.route('/display_products_2', methods=['GET'])
def get_products2():
    sort_by = request.args.get('sort_by')
    if sort_by == 'price_asc':
        sorted_products = sorted(products2, key=lambda x: x['price'])
    elif sort_by == 'price_desc':
        sorted_products = sorted(products2, key=lambda x: x['price'], reverse=True)
    else:
        sorted_products2 = products2  # Default sorting by product ID

    return render_template('products2.html', sorted_products=sorted_products2)

@app.route('/display_product2/<int:product_id>', methods=['GET'])
def get_product2(product_id):
    product = next((p for p in products2 if p['id'] == product_id), None)
    if product:
        return jsonify(product)
    else:
        return jsonify({'message': 'Product not found'}), 404
    
@app.route('/display_products_3', methods=['GET'])
def get_products3():
    sort_by = request.args.get('sort_by')
    if sort_by == 'price_asc':
        sorted_products = sorted(products3, key=lambda x: x['price'])
    elif sort_by == 'price_desc':
        sorted_products = sorted(products3, key=lambda x: x['price'], reverse=True)
    else:
        sorted_products3 = products3  # Default sorting by product ID

    return render_template('products3.html', sorted_products=sorted_products3)

@app.route('/display_product3/<int:product_id>', methods=['GET'])
def get_product3(product_id):
    product = next((p for p in products3 if p['id'] == product_id), None)
    if product:
        return jsonify(product)
    else:
        return jsonify({'message': 'Product not found'}), 404    
...
@app.route('/add_to_cart_2/<user_id>/<int:product_id>', methods=['POST'])
def add_to_cart_2(user_id, product_id):
    if user_id not in carts:
        carts[user_id] = {}

    product = next((p for p in products2 if p['id'] == product_id), None)
    if not product:
        return jsonify({'message': 'Product not found'}), 405
...
@app.route('/add_to_cart_3/<user_id>/<int:product_id>', methods=['POST'])
def add_to_cart_3(user_id, product_id):
    if user_id not in carts:
        carts[user_id] = {}
...
@app.route('/product2/<int:product_id>', methods=['GET'])
def display_product2(product_id):
    product = next((p for p in products2 if p['id'] == product_id), None)
    if product:
        return render_template('products2_detail.html', product=product)
    else:
        return jsonify({'message': 'Product not found'}), 404
    
@app.route('/product3/<int:product_id>', methods=['GET'])
def display_product3(product_id):
    product = next((p for p in products3 if p['id'] == product_id), None)
    if product:
        return render_template('products3_detail.html', product=product)
    else:
        return jsonify({'message': 'Product not found'}), 404    
